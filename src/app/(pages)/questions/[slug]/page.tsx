...
export default async function Question(params: { params: { slug?: string } }) {
  ...
  return (
    ...
    <div className='flex justify-center'>
      <div className="grid grid-cols-[400px,600px,200px,1fr] max-w-[1120px]">
        <YoutubeAsideList />
        <article className='flex-col w-[640px] shadow-[0_11px_9px_6px_rgba(0,0,0,0.3)] rounded-xl p-2 mt-5 h-fit -mr-10 -ml-10'>
          ...
          {question.answers && question.answers.length > 0 &&
            question.answers.map((answer, index) => (
              <section className="m-3 mt-5" key={answer.id}>
                ...
                {index !== question.answers.length - 1 && <hr className='mt-4 border-0 h-[1px] bg-gray-400 mx-5' />}
              </section>
            ))}
        </article>
      </div>
